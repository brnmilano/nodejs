...
app.post("/address/delete", async (req, res) => {
  ...
  try {
    // O método destroy é usado para excluir um endereço do banco de dados com base no ID fornecido
    // WHERE é usado para especificar a condição de exclusão,
    // garantindo que apenas o endereço com o ID correspondente seja removido.
    await Address.destroy({ where: { id } });

    res.redirect(`/users/edit/${UserId}`);
  } catch (error) {
    console.error("Erro ao excluir endereço:", error);
    res.status(500).send("Erro ao excluir endereço");
  }
});
